...
def retrieve_initial_bins_from_tree(
    ...
    while len(stack) > 0:
        ...
        if not _is_leaf(tree, node_id):
            left_child = tree.children_lef[node_id]
            right_child = tree.children_right[node_id]
            stack.append(right_child)
            stack.append(left_child)

            if _is_leaf(left_child) or _is_leaf(right_child):
                bin_thresholds.append(tree.threshold[node_id])
...
def _is_leaf(tree: DecisionTreeClassifier, node_id: int) -> bool:
    ...
    return tree.children_lef[node_id] == tree.children_right[node_id]
